...
Eine Formulierung kann immer genau auf eine Folge von Befehlen zurückgeführt werden, wie am Beispiel in Listing \ref{lst:mergen} gezeigt wird.
...
Die Schwierigkeit bei der Entwicklung des \gls{ape}-Services liegt im reibungslosen Verknüpfen der verschiedenen Services. Ein Schema der existierenden Schnittstellen ist in Abbildung\comment{fehlt} zu sehen. In den Kapiteln \ref{ssub:interpretation_von_git_commits} bis \ref{ssub:uebertragung_der_dateien} werden einige ausgewählte nichttriviale Aspekte der Schnittstellenimplementierung besprochen.
